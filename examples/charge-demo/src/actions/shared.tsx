...
/**
 * Demo-only Permit2 spender for LOCAL mode (no real charge-server).
 *
 * Permit2 requires the signed `spender` to equal the on-chain msg.sender
 * at settlement (PermitHash._hashWithWitness). In server mode the server
 * publishes its real settlement-signer address as
 * `methodDetails.permit2Spender`. In local mode there is no server and
 * no on-chain settlement (buildReceiptLocal synthesizes the receipt), so
 * we inject this fixed placeholder purely so the local typed-data sign +
 * EIP-712 recover round-trip is internally consistent. It is NOT a real
 * settlement signer and a credential built locally would NOT settle
 * against a real Permit2 (the spender wouldn't match any real caller).
 */
...
export function getSignerAddress(
  ...
  if (state.inPageAccount) return state.inPageAccount.address
  throw new Error(
    'No signer available. Connect MetaMask (recommended) or generate an in-page key (Transaction tab).',
  )
}
