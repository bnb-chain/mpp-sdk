 ...
 *
 * Dispatches by the selected credential type (hash / permit2 /
 * authorization) and produces the corresponding credential — broadcasting a
 * real BSC Testnet USDT transfer for `hash`, or wallet-signing EIP-712
 * typed data for `permit2` / `authorization`.
 */
...
export async function buildCredential(
  ...
  switch (state.credentialType) {
    ...
    case 'x402': {
      // The x402 tab never reaches this mppx pipeline — App.tsx dispatches it
      // to actions/x402.tsx (a different wire). Kept for switch exhaustiveness.
      throw new Error('x402 uses its own pipeline (actions/x402.tsx) — not the mppx builder.')
    }
    case 'authorization': {
      if (!preset.eip712) {
        throw new Error(
