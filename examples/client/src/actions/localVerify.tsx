...
export async function localVerify(
  ...
  // The challenge is server-issued: its HMAC uses the server's secret, which
  // is (correctly) not exposed to the client — the server re-verifies it on
  // submit. Recorded as an explicit 'skipped' line, not silently omitted.
  lines.push({
  ...
  switch (state.credentialType) {
    case 'x402':
      ...
      throw new Error('x402 uses its own pipeline (actions/x402.tsx) — not the mppx verifier.')
    case 'hash': {
