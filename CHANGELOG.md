...
- 7cad29f: Security (audit H01): the hash verifier's `hashFromPolicy` now defaults to `'strict_from'` — a hash credential must carry a `source` DID matching the on-chain `Transfer.from`, so a bystander can no longer claim a payer's transaction by racing its hash to the server (zero-cost order sniping). The `pay()` client automatically binds `source` to the paying account, so SDK-to-SDK flows keep working unchanged.
